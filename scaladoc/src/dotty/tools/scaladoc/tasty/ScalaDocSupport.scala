  ...
  def parseComment(using Quotes, DocContext)(docstring: String, tree: reflect.Tree): Option[Comment] =
    val (commentString: String, commentOwner: reflect.Symbol) =
      if tree.symbol.isClassDef || tree.symbol.owner.isClassDef then
        ...
        // A member without a docstring of its own inherits the comment of the
        // nearest overridden symbol that has one (see `CommentExpander.superComment`).
        // Links in such a comment must be resolved in the context of the symbol
        // the comment was written for, not the inheriting member, which may not
        // see the same names.
